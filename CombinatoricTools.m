(* ::Package:: *)
Clear["CombinatoricTools`*"];

BeginPackage["CombinatoricTools`"];
...
RandomSetPartition[0, k_Integer] := If[k == 0, {{}}, {}];
RandomSetPartition[1, k_Integer] := If[k == 1, {{1}}, {}];
RandomSetPartition[n_Integer,1] := {Range@n};
RandomSetPartition[n_Integer,n_Integer] := List /@ (Range@n);

(* Memoized Stirling numbers. Quotients of these leads to numerical issues.
perhaps there is a nice recursion for the quotients themselves? *)
stirling2mem[1, k_Integer] := N[Boole[k == 0]];
stirling2mem[n_Integer, 1] := N[1];
stirling2mem[n_Integer, n_Integer] := N[1];
stirling2mem[n_Integer, k_Integer] := 
  stirling2mem[n, k] = k*stirling2mem[n - 1, k] + stirling2mem[n - 1, k - 1];

(* Memoized stirling quotient. *)
squot[n_Integer,k_Integer]:=squot[n,k]=N[StirlingS2[n-1,k-1]/StirlingS2[n,k]];


(* Helper function for random set partition *)
(* 
From https://mathematica.stackexchange.com/questions/276505/speed-up-random-set-partition-code
*)
rSow[n_Integer,n_Integer]:=Scan[Sow[#,#]&,Range[n]];
rSow[n_Integer,k_Integer]:=If[
	RandomReal[]<=squot[n,k],
		rSow[n-1,k-1];Sow[n,k];
	,
		rSow[n-1,k];Sow[n,RandomInteger[{1,k}]
	];
];
RandomSetPartition[n_Integer,k_Integer]:=Flatten[Last[Reap[rSow[n,k],Range[k]]],1];


(* Old inefficient code below. *)
(*
RandomSetPartition[n_Integer, k_Integer] :=
   (* Either n is a singleton block, or, uniformly in an existing block. *)
   If[RandomReal[] <= StirlingS2[n - 1, k - 1] / StirlingS2[n, k],
    Append[RandomSetPartition[n - 1, k - 1], {n}]
    ,
    (* Otherwise, insert n into a set-partition with k blocks already. 
    The block is chosen uniformly at random. *)
    MapAt[Append[#, n] &, RandomSetPartition[n - 1, k], RandomInteger[{1, k}]]
];
*)

(* We use the Stirling numbers as weights, to determine number of parts. *)
RandomSetPartition[n_Integer] := With[{
    k = RandomChoice[
      Table[StirlingS2[n, i]/BellB[n], {i, n}] -> Range[n]]
    }
   ,
   RandomSetPartition[n, k]
];
